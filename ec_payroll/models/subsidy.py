...
class hrContract(models.Model):
	...
	def create_leaves_entries_values(self, date_start, date_stop,work_entry_type_id,subsidy_id):
		...
		for contract in self:
			...
			# Attendances
			for interval in attendances[0]._items:
...
class hrSubsidy(models.Model):
	...
	type_subsidy = fields.Selection([('maternity', 'Maternidad'),
									 ('disease', 'Enfermedad'),
									 ('accident', 'Accidente')], string=u'Tipo de Subsidio', required=True)
	...
	def change_date(self):
		...
		if self.date_start and self.type_subsidy=='maternity':	
			self.date_end=self.date_start +  relativedelta(days=80) #aqui estaba 84 pero los dias no cuadran xq debe contar el primer dia de la fecha
	...
	def get_conf_work_entry(self):
		if self.env.company.work_entry_type:
			return self.env.company.work_entry_type
		else:
			return False
	...
	def get_conf_work_entry_type(self):
		if self.type_subsidy=='maternity':
			if self.env.company.maternity_entry_type:
				return self.env.company.maternity_entry_type
			else:
				return False
			
		if self.type_subsidy=='disease':
			if self.env.company.disease_entry_type:
				return self.env.company.disease_entry_type
			else:
				return False

		if self.type_subsidy=='accident':
			if self.env.company.accident_entry_type:
				return self.env.company.accident_entry_type
			else:
				return False
	...
	def get_days(self,date_start,date_end):
		...
		if days>84 and self.type_subsidy=='maternity':
			self.date_end=None
			raise UserError(_(u'El máximo número de días de Maternidad es 80') )
